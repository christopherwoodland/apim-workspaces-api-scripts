...
param(
    ...
    [Parameter()]
    [string]$BackendUrl = "https://postman-echo.com",

    [Parameter()]
    [string]$ApiId = "echo-api",

    [Parameter()]
    [string]$ApiDisplayName = "Echo API",

    [Parameter()]
    [string]$ApiPath = "echo",

    [Parameter()]
    [string]$ProductId = "echo-tests",

    [Parameter()]
    [string]$ProductDisplayName = "Echo Tests",

    [Parameter()]
    [string]$ProductDescription = "Test product for the echo API",
...
function Invoke-WorkspaceRest {
    ...
    try {
        if ($null -ne $BodyObject) {
            ...
            $bodyFile = Join-Path $env:TEMP ("apim-echo-body-" + [guid]::NewGuid().ToString() + ".json")
...
$openApiDocument = @{
    ...
    info = @{
        ...
        version = "1.0.0"
        description = "A simple echo API for workspace testing"
    }
    servers = @(
        @{
            url = $BackendUrl
...
[void](Wait-WorkspaceResource -ChildPath "apis/$ApiId" -ExpectName $ApiId -TimeoutSeconds $VerificationTimeoutSeconds)
...
$publicApiUrl = ($apimGatewayUrl.TrimEnd('/') + "/" + $ApiPath + "/get?source=apim")
...
if ($CreateProduct) {
    ...
    [void](Invoke-WorkspaceRest -Method "PUT" -ChildPath "products/$ProductId" -BodyObject $productBody)
